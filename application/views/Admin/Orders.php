      ...
      fetch("<?= base_url('updateOrderStatus') ?>", {
        ...
        .then(data => {
          if (data.success) {
            ...
            if (row) {
              // Change status badge
              let statusCell = row.querySelector("td[data-label='Status']");
              if (statusCell) {
                statusCell.innerHTML = `<span class="status-badge status-available">Available</span>`;
              }

              // Remove Add to Stock button → replace with dropdown + actions
              let actionsCell = row.querySelector("td[data-label='Actions']");
              if (actionsCell) {
                actionsCell.innerHTML = `
              <select name="status" class="form-select form-select-sm"
                onchange="updateStatus(this.value, '${invoiceId}', '${itemName}')">
              ...
              // ✅ Add green row highlight
              row.classList.remove("highlight-today", "highlight-tomorrow", "highlight-overdue");
              row.classList.add("highlight-available");
            }
          } else {
            Swal.fire({
              icon: 'error',
